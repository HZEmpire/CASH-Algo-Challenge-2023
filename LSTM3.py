...
class AlgoEvent:
    ...
    def on_bulkdatafeed(self, isSync, bd, ab):
        ...
        if self.LSTM_prediction >= 0.8 and todayclose[-1] > todayclose[-2]:
            self.evt.consoleLog('True prediction')
        elif self.LSTM_prediction < 0.2 and todayclose[-1] < todayclose[-2]:
            self.evt.consoleLog('True prediction')
        elif self.LSTM_prediction >= 0.2 and self.LSTM_prediction < 0.8:
            self.evt.consoleLog('Not T/F')
        else:

...
class AlgoEvent:
    ...
    # 若预测值为上涨则开仓
        ...
        if position > 0:
            ...
            elif self.LSTM_prediction >= 0.7 and self.LSTM_predictio < 0.8:
                self.evt.consoleLog('Buy')
            ...
            # 减仓 30%
                ...
                self.doit(self.myinstrument, -1, self.ref, position )
            elif self.LSTM_prediction <= 0.4 and self.LSTM_prediction > 0.2:
                self.evt.consoleLog('Sell')
                self.doit(self.myinstrument, -1, self.ref, position * 0.4)
                
            else:
                self.evt.consoleLog('Sell')
                self.doit(self.myinstrument, -1, self.ref, position * 0.2)
        if position < 0:
            # 减仓 5%
            if self.LSTM_prediction <= 0.2:
                self.evt.consoleLog('Sell')
                self.doit(self.myinstrument, -1, self.ref, position * 0.05)
            # 加仓 30%
            elif self.LSTM_prediction >= 0.8:
                self.evt.consoleLog('Buy')
